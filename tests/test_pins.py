...
def test_pin_unpin_ordering():
    ...
    # initial order: third, second, first (most recent at front)
    ids_before = [it.content for it in hs.items]
    assert ids_before[0] == 'third'
    # pin 'second'
    hs.pin_item(b.id)
    # pinned should appear at top
    assert hs.items[0].id == b.id
    # unpin and ensure it moves after pinned area
    hs.unpin_item(b.id)
    # now top should be most recent non-pinned (third)
